# Portal: Technical Architecture & Design Document

Portal is a lightweight, high-performance desktop utility designed to facilitate seamless file sharing and clipboard synchronization between computers on the same local area network (LAN) with known IP addresses. 
...
Below is the design mockup representing the visual interface of **Portal**:

![Portal UI Mockup](/Users/jinwoohwangbo/.gemini/antigravity-cli/brain/0ccfdddd-abe9-4e99-bcdb-4202ffb6e3dd/app_ui_mockup_1783568153145.jpg)
...
Since Portal operates in a peer-to-peer (P2P) fashion on a local network, every instance of the application acts as both a **Server** (listening for incoming files and clipboard updates) and a **Client** (initiating outgoing connections, file uploads, and clipboard transfers).
...
* **Response (200 OK)**:
  ```json
  {
    ...
    "destination_path": "/Downloads/Portal"
  }
  ```
...
> 3. **Directory Lock**: The server module must only write incoming files to a designated "Safe Folder" (e.g., `~/Downloads/Portal`) and sanitize filenames to prevent path traversal attacks (e.g. `../../etc/passwd`).
